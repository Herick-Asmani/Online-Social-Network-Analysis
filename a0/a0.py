...
consumer_key = 'DJ3DvEPslGXH0ALyPMs1zBWtV'
consumer_secret = 'kXwHa0FM8Q8nwdc71I44rneb38854pAMuPZDj4SkoEflfdkrVZ'
access_token = '1205534714-PN56gE7fUKqbpp5Pbnc2ZCwjkzQuvbLDYg0FjHX'
access_token_secret = 'CSoayPduZscOKCaSgEcycLJUCgc5MAdewat8FuAfgUkRM'
...
def read_screen_names(filename):
    ...
    """
    path = 'C:/Users/Herick/Desktop/MS/CS579 Online Social Network Analysis/Assignments/Herick-Asmani/a0/candidates.txt'
...
def get_users(twitter, screen_names):
    ...
    [6253282, 783214]
    """
    users = robust_request(twitter, 'users/lookup', {'screen_name': screen_names}, max_tries=5)
    return users
...
def get_friends(twitter, screen_name):
    ...
    """
    friends = []
    friends_request = robust_request(twitter, 'friends/ids',{'screen_name':screen_name, 'count': 5000}, max_tries=5)
    friends = sorted([i for i in friends_request])
...
def add_all_friends(twitter, users):
    ...
    >>> users[0]['friends'][:5]
    [695023, 1697081, 8381682, 10204352, 11669522]1
    """
    for user in users:
...
def friend_overlap(users):
    ...
    for i in range(len(users)):
        for j in range(i+1, len(users)):
            union = set(users[i]['friends']) & set(users[j]['friends'])
            overlap.append((users[i]['screen_name'], users[j]['screen_name'], len(union)))
    friend_overlap = sorted(overlap, key=lambda x: -x[2])
...
def followed_by_hillary_and_donald(users, twitter):
    ...
    for user in range(len(users)):
        ...
        if users[user]['screen_name'] == "realDonaldTrump":
            trump_id = user
    union = set(users[hillary_id]["friends"]) & set(users[trump_id]["friends"])
    commonuser_request = robust_request(twitter, "users/lookup", {"user_id" : union})
    
    return str([u['screen_name'] for u in commonuser_request])
    ...
    for user in users:
        ...
        for friend_id in user['friends']:
            if friend_counts[friend_id] > 1:
                ...
                graph.add_edge(user['screen_name'],friend_id)
...
def draw_network(graph, users, filename):
    ...
    """
    user_labels = {user['screen_name']:user['screen_name'] for user in users}

    nx.draw_networkx(graph,node_size = 30, alpha = 0.45, width = 0.1, edge_color = 'k', labels = user_labels, font_size = 6)
